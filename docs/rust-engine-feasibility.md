...
### 1.35 体积账：换 Rust 之后**只剩模型大**（实测，压缩后）

Release 资源是 zip，所以「gzip 后」才是用户真正下载的体积。实测（`gzip -9`）：

| 图 | 原始 | gzip | 压缩率 |
|---|---|---|---|
| 编码器 fp32 | 343.4 MB | 318.7 MB | 0.93 |
| 编码器 int8 | 86.9 MB | 72.3 MB | 0.83 |
| 解码器 fp32 | 117.4 MB | 108.9 MB | 0.93 |
| 解码器 int8 | 29.6 MB | 23.2 MB | 0.78 |
| 检测器 fp32 | 94.8 MB | 57.6 MB | 0.61 |
| 检测器 int8 | 53.6 MB | 36.4 MB | 0.68 |
| **三个模型合计 fp32** | **555.6 MB** | **485.2 MB** | 0.87 |
| **三个模型合计 int8** | **170.1 MB** | **131.9 MB** | 0.78 |

换 Rust 之后，归档里剩下的**只有这些**：

| 组成 | 体积（gzip 后） | 说明 |
|---|---|---|
| 三个 ONNX 模型 | **132 MB**（全 int8）/ **485 MB**（全 fp32） | 唯一的大头 |
| ONNX Runtime 动态库 | **33.2 MB**（实测 macOS arm64 的 `libonnxruntime.1.30.0.dylib`） | `ort` 的预编译版；静态链接就并进二进制 |
| Rust 二进制 | **~2 MB**（用现有 `arale-native` 实测代理） | zip/rar/7z 解包器那个 |
| 字符表 `vocab.txt` | 24 KB | 替换掉 248 MiB 的 UniDic |
| NFKC 归一化表 | ~0.5–1 MB（估算，编译进二进制） | `unicode-normalization` |
| **合计** | **≈ 170 MB**（int8） / **≈ 520 MB**（fp32） | 对比现在 **741 MiB** |

**消失的东西**（现在占归档约 600 MiB / 解包 1.6 GB）：CPython 47 MB、torch 582 MB、unidic_lite 248 MB、
cv2 119 MB、transformers 110 MB、scipy 98 MB、sympy 72 MB、numpy/Pillow/tokenizers/…

**两条路线，差别是 4 倍还是 1.3 倍**——取决于 int8 能不能过 §4 的验收：

- int8 过关 → 归档 **≈ 170 MB（现在的 1/4）**；
- int8 不过关、只能 fp32 → 归档 **≈ 520 MB**，比现在只小 30%（因为 fp32 的浮点权重几乎压不动，0.93）。

所以**「int8 到底行不行」是整个项目最值钱的一个问号**。

**一个更稳的折中（推荐）**：检测器留 **fp32**、只量化识别端。理由是风险不对称——
检测器输出的是**框**，量化错了会让文字层错位（`§4` 的 IoU 指标直接挂）；识别端输出的是**文字**，量化错了好验证。
而检测器 fp32 只要 57.6 MB（gz）：

| 方案 | 模型（gz） | + 运行时 | 归档 | 风险 |
|---|---|---|---|---|
| 全 int8 | 131.9 MB | 35 MB | **≈ 167 MB** | 框 + 文字都可能漂 |
| 检测器 fp32 + 识别 int8 | 153.1 MB | 35 MB | **≈ 188 MB** | 只有文字可能漂（好验收） |
| 全 fp32 | 485.2 MB | 35 MB | ≈ 520 MB | 基本无风险 |

**还有一个产品选项**：模型不随扩展下载，改成**首次 OCR 时按需下载**——应用已经有带 sha256 校验的下载器与进度 UI
（就是扩展安装器本身）。那样扩展本体只有 **~35 MB**（二进制 + ORT + 词表），132 MB 模型按需拉。
代价是第一次识别要等一次下载，「离线可用」从「装着就离线」变成「下过就离线」。

> 没测的：`tract`（纯 Rust 推理，省掉 33 MB 的 ORT 动态库）比 ORT 慢多少、算子与量化支持够不够，**未测**。
